...
for answer in shouldHaveMorePollOptionsVotes {
    if answer == true {
        yes += 1
...
var yes1 = 0
var no1 = 0
for answer in shouldMascotChangeVotes {
    if answer == true {
        yes1 += 1
    } else {
        no1 += 1
...
var yes2 = 0
var no2 = 0
for answer in shouldInstallCoffeeVendingMachineVotes {
    if answer == true {
        yes2 += 1
    } else if answer == false {
        no2 += 1
...
func printResults(for issue: String, with votes: [Bool]) {
    var yes = 0
    var no = 0
    ...
    print("\(issue), \(yes) yes, \(no) no")
}

printResults(for: "Should we install Coffee Vending Machines?", with: shouldInstallCoffeeVendingMachineVotes)
